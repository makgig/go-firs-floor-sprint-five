...
func (t Training) Calories() float64 {
	// вставьте ваш код ниже
	return 0 // Пока возвращаем 0, так как этот метод будет переопределяться
}
...
type InfoMessage struct {
	...
	TrainingType string  // тип тренировки
	Duration     float64 // длительность тренировки в минутах
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	speed := r.meanSpeed()
	return ((CaloriesMeanSpeedMultiplier*speed + CaloriesMeanSpeedShift) * r.Weight * r.Duration.Hours() * MinInHours) / MInKm
}
...
func (w Walking) Calories() float64 {
	...
	return ((CaloriesWeightMultiplier*w.Weight + (speedMS*speedMS/heightM)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
}
